    ...
    if (answerId.value !== 0) {
        ...
        if (question.value.displayOrder >= 4) {
            try {
                ...
                // Add the fade-out animation to hide the current question
                const element = document.querySelector('.fade-in');
                if (element) {
                    element.classList.add('fade-out');
                    // After the animation ends, navigate to the next question
...
<template>
    ...
    <div v-if="question.id == 0"></div>

    <div v-else class="col-12 container-relative">
        <div class="row fade-in">
            <div class="col-12 col-sm-6 text-white d-flex position-relative question-container" :style="{'background-color': question.bgColor}">
                <img src="/question-img.png" alt="Une mère et sa fille en super-héros" class="img-fluid position-absolute question-bg-image">
                <div class="my-auto question-label-container" style="padding-bottom: 100px">
                    <div>
                        <img :src="'/0'+question.displayOrder+'.png'" class="mt-sm-5 question-number" :alt="`Question n°` + question.displayOrder"/>
                    </div>
                    <div>
